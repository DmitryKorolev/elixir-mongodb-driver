defmodule Mongo.MongoDBConnection.UtilsTest do
  ...
  describe "maybe_restrict_versions/1" do
    ...
    @tag @tls13_tag
    test "restricts versions to exclude TLS 1.3 for a TLS-1.2-only cipher (charlist)" do
      opts = [ciphers: [~c"AES256-SHA"]]

      {result, log} = with_log(fn -> Utils.maybe_restrict_versions(opts) end)

      assert result[:versions] == List.delete(supported(), @tls13)
      assert @tls13 not in result[:versions]
  ...
  defp supported(), do: :ssl.versions()[:supported]
end
