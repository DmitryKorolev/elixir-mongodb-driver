defmodule Mongo.MongoDBConnection.Utils do
  ...
  def maybe_restrict_versions(ssl_opts) do
    with ciphers when is_list(ciphers) and ciphers != [] <- ssl_opts[:ciphers],
      ...
      Logger.info("restricting TLS versions to exclude #{inspect(@tls13)} because supplied :ciphers do not include any TLS 1.3 suite")
      Keyword.put(ssl_opts, :versions, List.delete(supported, @tls13))
    else
      _ -> ssl_opts
    end
  end
  ...
  defp to_cipher_charlist(c) when is_map(c) or is_tuple(c) do
    :ssl.suite_to_str(c)
  rescue
    _ -> nil
  end
